...
import XbtnCircle from "../../../images/loginpage/x-button-circle.svg";
...
const CreateNewJob = ({ exitEdit, isDirectJob = true, mentorId = null, refreshFetch = null }) => {
    ...
    const createNewOpenJob = async (e) => {
        try {
            const newJobData = await fetch(`${import.meta.env.VITE_API_URL}/api/job`, {
                method: "POST",
                headers: {
    ...
    const createNewDirectJob = async (e) => {
        ...
        try {
            ...
            const newJob = await newJobData.json();
            const newDirectAplicationData = await fetch(`${import.meta.env.VITE_API_URL}/api/company/application/${newJob._id}/${mentorId}`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`,
